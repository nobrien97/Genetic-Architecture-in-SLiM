...
# rsample <- as.character(runif(4, 1, (2^62 - 1)))
...
foreach(i=rsample) %:%
  foreach(j=((PBSIND*6) - 5):(PBSIND*6)) %dopar% {
    ...
    slim_out <- system(sprintf("/home/$USER/SLiM/slim -d seed=%s -d rregion=%s -d Ne=%i -d QTL_cov=%f -d pleiorate=%f -d delmu=%f -d rwide=%s -d modelindex=%i /home/$USER/SLiM/Scripts/null2T100L_june_restruct2_opt_nomat.slim", 

...
foreach(i=rsample) %:%
  foreach(j=((PBSIND*5) - 4):(PBSIND*5)) %dopar% {
    ...
    slim_out <- system(sprintf("/home/$USER/SLiM/slim -s %s -d rregion=%s -d Ne=%i -d QTL_cov=%f -d pleiorate=%f -d delmu=%f -d rwide=%s -d modelindex=%i /home/$USER/SLiM/Scripts/null2T100L_june_restruct2_opt.slim", 

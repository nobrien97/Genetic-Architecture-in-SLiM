##############################################################################################################
#  Final Null independent assortment model script with arguments coming from latin square, for use on cluster
##############################################################################################################
...
foreach(i=rsample) %:%
  foreach(j=PBSIND) %dopar% {
    ...
    slim_out <- system(sprintf("/home/$USER/SLiM/slim -s %s -d Ne=%i -d rwide=%s -d pleio_cov=%f -d pleiorate=%f -d delmu=%f -d nloci=%i -d locisigma=%f -d delchr=%i -d modelindex=%i /home/$USER/SLiM/Scripts/null_recom_8T100L.slim", 

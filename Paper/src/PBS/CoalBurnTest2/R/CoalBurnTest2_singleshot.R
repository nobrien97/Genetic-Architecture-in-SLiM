...
seeds <- read.csv(paste0(HOME, "/tests/CoalBurnTest2/R/seeds_coalburntest.csv"), header = T)
combos <- read.csv(paste0(HOME, "/tests/CoalBurnTest2/R/lhc_coalburntest.csv"), header = T)
...
time <- system.time(
  ...
  if(combos$tree[i]){
    ...
    system(sprintf("$HOME/.conda/envs/msslim/bin/python3.9 $HOME/tests/CoalBurnTest2/R/burnin.py %i %s",
                  i,
                  as.character(seeds$Seed[j]),
                  intern=T))

    # Run SLiM
    slim_out <- system(sprintf("$HOME/slim/slim -s %s -d Ne=%i -d rwide=%f -d modelindex=%i $HOME/tests/CoalBurnTest2/slim/polygen_maint_coalload.slim",
                                as.character(seeds$Seed[j]),
    ...
    slim_out <- system(sprintf("$HOME/slim/slim -s %s -d Ne=%i -d rwide=%f -d modelindex=%i $HOME/tests/CoalBurnTest2/slim/polygen_maint.slim",
                            as.character(seeds$Seed[j]),
...
write.table(time, paste0("/scratch/user/uqnobri4/CoalBurnTest2/time_", seeds$Seed[j], "_", i), 
